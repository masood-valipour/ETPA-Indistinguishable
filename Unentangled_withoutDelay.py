...
"""
def Gaussian_Unentangled_withoutDelay(params: Tuple[float, float]) -> callable:
    OmegaA, OmegaB = params
    N_PHI = 1 + 2 * (OmegaA * OmegaB)/(OmegaA**2 + OmegaB**2)
    N_PHI *= 4
...
def task(Gamma_e: float) -> pd.DataFrame:
    t0 = time()
    res = optimize(	P_negated(Gamma_e, Gamma_f, Gaussian_Unentangled_withoutDelay, tol = 1e-15, limit = 200), 
                    sp_bounds = np.array([[0, 10],[1e-9,1],[1e-9,2]]),
                    p_bounds = ((None,None),(0,None),(0,None)), 
                     N = 150)
    res['t_max'], res['OmegaA'], res['OmegaB']=  res.x
    res['P_max'] = -res['fun']
    res['ComputTime'] = time()-t0
    res['Gamma_e'] = Gamma_e
    res = pd.DataFrame([res])
    res = res[['P_max', 'OmegaA', 'OmegaB', 't_max', 'Gamma_e',  'number_of_fails', 'ComputTime'] ]
...
def P( Gamma_e, Gamma_f= 1, w_fe = 0, w_eg = 0, w_a = 0, w_b = 0):
    def inner(params):
        t, OmegaA, OmegaB = map(mp.mpf, params)
        MuA = 0
        MuB = 0
...
def task(Gamma_e: float) -> pd.DataFrame:
    t0 = time()
    # d_a, d_f = 0.102041, -0.714286
    d_a, d_f = 0, 2.7778
    w_fe = mp.mpf(0.5)*d_a
    w_eg = -mp.mpf(0.5)*d_a
    w_01 = -mp.mpf(0.5)*d_f
    w_02 = mp.mpf(0.5)*d_f
    res = optimize(     P (Gamma_e,w_fe = w_fe, w_eg = w_eg, w_a = w_01, w_b= w_02), 
                    sp_bounds = np.array([[0, 10],[0,3],[0,3]]),
                    p_bounds = ((None,None),(1e-9,None),(1e-6,None)), 
                     N = 10)
    res['t_max'], res['OmegaA'], res['OmegaB']=  res.x
    res['P_max'] = -res['fun']
    res['ComputTime'] = time()-t0
    ...
    res['delta_f'] = d_f
    res = pd.DataFrame([res])
    res = res[['P_max', 'OmegaA', 'OmegaB', 't_max', 'Gamma_e', 'delta_a', 'delta_f', 'number_of_fails','mp.dps', 'ComputTime'] ]
    res.to_csv(fileName, mode='a', header=True, index=False)
    return
...
fileName = get_unique_filename('P_Gamma_Unentangled_withoutDelay_test.csv')

Gamma_f = 1

Gamma_e_range = np.logspace(-3,4,28,endpoint=False)
# Gamma_e_range = [316.227766, 1, 0.1]
Gamma_e_range = [1,1,1]
Gamma_e_range = [5,5,5]
# Gamma_e_range = Gamma_e_range[0:10]
