...
inputFile = f'P_heatMap_Gamma_Delta_Unentangled.csv'
...
# ax1.set_title(r'Unentangled, with Delay $\delta_f = 0$', fontsize=14, pad=10)


ax2 = fig.add_subplot(gs[35:65,0:100])
inputFile = f'HeatMap_deltas_UnEnt_Delay.csv'
inputFile = './DataSets/' + inputFile
df = pd.read_csv(inputFile)
df = df[df['P_max'] != 'P_max'].astype('float')
df.sort_values(by=['delta_a','Gamma_e'], ascending=True, inplace=True)
df.reset_index(drop=True, inplace=True)
df = df[df['delta_a']<20.5]
...
ax2.set_yticks(np.linspace(0,25,5,endpoint=False))
ax2.tick_params(axis='both', color="#CBFAF4", which='both')   # or any color you want
...
ax2.text(0.03, 0.97, '(b)', transform=ax2.transAxes,
            fontsize=10, verticalalignment='top', color="#CBFAF4", weight='bold')
# choose contour levels (same idea as your imshow code)
Z = P_grid
levels = np.percentile(Z, [32,90])  
cs = ax2.contour(
    Gamma_e,
    delta_a_unique,
    Z,
...
outputFile = './Plots/'+ 'P_heatMap_Gamma_deltaA_UnEnt.png'
